...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/29 12:54:42 by apreymon          #+#    #+#             */
/*   Updated: 2020/06/27 12:32:46 by apreymon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_wall_h(t_game *g, float x, float y)
{
	...
	line = (int)g->p.y;
	width = (ft_strlen(g->set.map[line]) - 1);
	...
	if (y > g->set.map_y - 1)
		...
		y = 0;
	if (x >= ft_strlen(g->set.map[(int)y]))
		x = ft_strlen(g->set.map[(int)y]) - 1;
